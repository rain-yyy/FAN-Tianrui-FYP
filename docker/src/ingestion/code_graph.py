...
if __name__ == "__main__":
    ...
    # ── Config ───────────────────────────────────────────────────────────
    # Use docker/ as the repo root so that relative paths include the "src/"
    # prefix, matching the "from src.X import Y" style used throughout the
    # codebase.  This ensures IMPORTS edges resolve correctly.
    _DOCKER_ROOT = _Path(__file__).parent.parent.parent          # docker/
    _BUILD_ROOT  = str(_DOCKER_ROOT)                             # repo root for the builder
    _SRC_DIR     = _DOCKER_ROOT / "src"
    ...
    # ── Discover files ────────────────────────────────────────────────────
    _py_files  = _glob.glob(str(_SRC_DIR / "**" / "*.py"),  recursive=True)
    _ts_files  = _glob.glob(str(_DOCKER_ROOT / "frontend" / "**" / "*.ts"),  recursive=True)
    _tsx_files = _glob.glob(str(_DOCKER_ROOT / "frontend" / "**" / "*.tsx"), recursive=True)
    _all_files = _py_files + _ts_files + _tsx_files

    print(f"\n{_HEAD}═══ Code Graph — Smoke Test ═══{_RST}")
    print(f"  Repo root : {_BUILD_ROOT}")
    print(f"  Files     : {len(_py_files)} .py  |  {len(_ts_files)} .ts  |  {len(_tsx_files)} .tsx\n")

    # ── Phase 1: Build graph ──────────────────────────────────────────────
    print(f"{_HEAD}[1] Graph construction{_RST}")
    try:
        builder = CodeGraphBuilder()
        g = builder.build_graph(_BUILD_ROOT, _all_files)
        _check("build_graph() completes without exception", True)
    except Exception as _e:
        _check("build_graph() completes without exception", False, str(_e))
        print("\n  Cannot continue — aborting.")
        sys.exit(1)

    n_nodes = g.number_of_nodes()
    n_edges = g.number_of_edges()
    _check("Graph has nodes",   n_nodes > 0,  f"{n_nodes} nodes")
    _check("Graph has edges",   n_edges > 0,  f"{n_edges} edges")

    _type_counts  = _Counter(d.get("type")  for _, d      in g.nodes(data=True))
    _etype_counts = _Counter(d.get("type")  for _, _, d   in g.edges(data=True))

    print(f"\n  Node breakdown  : {dict(_type_counts)}")
    print(f"  Edge breakdown  : {dict(_etype_counts)}")

    # ── Phase 2: Node-quality checks ─────────────────────────────────────
    print(f"\n{_HEAD}[2] Node quality{_RST}")

    _check("Has FILE nodes",      _type_counts.get("file", 0) > 0,
           f"{_type_counts.get('file', 0)} file nodes")
    _check("Has CLASS nodes",     _type_counts.get("class", 0) > 0,
           f"{_type_counts.get('class', 0)} class nodes")
    _check("Has FUNCTION nodes",  _type_counts.get("function", 0) > 0,
           f"{_type_counts.get('function', 0)} function nodes")

    # Every non-file node must have a qualified_name and start_line
    _bad_qname = [nid for nid, d in g.nodes(data=True)
                  if d.get("type") != "file" and not d.get("qualified_name")]
    _bad_line  = [nid for nid, d in g.nodes(data=True)
                  if d.get("type") not in ("file", None) and not d.get("start_line")]
    _check("All non-file nodes have qualified_name", len(_bad_qname) == 0,
           f"{len(_bad_qname)} missing" if _bad_qname else "")
    _check("All non-file nodes have start_line",     len(_bad_line)  == 0,
           f"{len(_bad_line)} missing"  if _bad_line  else "")

    # Node IDs must not contain raw code snippets (quick sanity: no newlines in ID)
    _bad_ids = [nid for nid in g.nodes if "\n" in nid]
    _check("No newlines in node IDs (byte-offset bug check)", len(_bad_ids) == 0,
           f"{len(_bad_ids)} bad IDs" if _bad_ids else "")

    # Class methods must use qualified names  e.g.  "MyClass.my_method"
    _method_nodes = [(nid, d) for nid, d in g.nodes(data=True)
                     if d.get("function_type") == "method"]
    _methods_with_class = [nid for nid, d in _method_nodes
                           if "." in (d.get("qualified_name") or "")]
    if _method_nodes:
        _ratio = len(_methods_with_class) / len(_method_nodes)
        _check("Methods have class-prefixed qualified names",
               _ratio >= 0.9,
               f"{len(_methods_with_class)}/{len(_method_nodes)} ({_ratio:.0%})")
    else:
        print(f"  -  No method nodes found (skip method qualified-name check)")

    # Signatures must be non-empty for functions
    _funcs = [(nid, d) for nid, d in g.nodes(data=True) if d.get("type") == "function"]
    _no_sig = [nid for nid, d in _funcs if not d.get("signature")]
    _check("All function nodes have a signature",
           len(_no_sig) == 0,
           f"{len(_no_sig)} missing" if _no_sig else "")

    # Visibility must be one of the valid values
    _valid_vis = {"public", "protected", "private", "internal", None}
    _bad_vis = [nid for nid, d in g.nodes(data=True)
                if d.get("visibility") not in _valid_vis]
    _check("All visibility values are valid",  len(_bad_vis) == 0,
           f"{len(_bad_vis)} invalid" if _bad_vis else "")

    # ── Phase 3: Edge-quality checks ─────────────────────────────────────
    print(f"\n{_HEAD}[3] Edge quality{_RST}")

    _check("Has CONTAINS edges", _etype_counts.get("contains", 0) > 0,
           f"{_etype_counts.get('contains', 0)}")
    _check("Has IMPORTS edges",  _etype_counts.get("imports",  0) > 0,
           f"{_etype_counts.get('imports', 0)}")
    _check("Has CALLS edges",    _etype_counts.get("calls",    0) > 0,
           f"{_etype_counts.get('calls', 0)}")

    # Every edge endpoint must exist as a node
    _dangling = [(u, v) for u, v in g.edges()
                 if not g.has_node(u) or not g.has_node(v)]
    _check("No dangling edge endpoints", len(_dangling) == 0,
           f"{len(_dangling)} dangling" if _dangling else "")

    # All edges must have a 'type' attribute
    _no_etype = [(u, v) for u, v, d in g.edges(data=True) if not d.get("type")]
    _check("All edges have a type attribute", len(_no_etype) == 0,
           f"{len(_no_etype)} missing" if _no_etype else "")

    # CONTAINS should form a forest (each non-root node has exactly 1 parent)
    _contains_in = _Counter(
        v for _, v, d in g.edges(data=True) if d.get("type") == "contains"
    )
    _multi_parent = {v: c for v, c in _contains_in.items() if c > 1}
    _check("No node has multiple CONTAINS parents (DAG check)",
           len(_multi_parent) == 0,
           f"{len(_multi_parent)} nodes with >1 parent" if _multi_parent else "")

    # ── Phase 4: Semantic-edge spot checks ───────────────────────────────
    print(f"\n{_HEAD}[4] Semantic edges{_RST}")

    _inherits_edges = [(u, v) for u, v, d in g.edges(data=True) if d.get("type") == "inherits"]
    _impls_edges    = [(u, v) for u, v, d in g.edges(data=True) if d.get("type") == "implements"]
    print(f"  INHERITS   : {len(_inherits_edges)}")
    print(f"  IMPLEMENTS : {len(_impls_edges)}")
    for u, v in _inherits_edges:
        print(f"    {u.split(':',1)[-1]}  →  {v.split(':',1)[-1]}")

    # ── Phase 5: Spot-check a known class/method ─────────────────────────
    print(f"\n{_HEAD}[5] Known-symbol spot check{_RST}")

    def _find(type_: str, name: str):
        return [(nid, d) for nid, d in g.nodes(data=True)
                if d.get("type") == type_ and d.get("name") == name]

    for _sym, _t in [
        ("CodeGraphBuilder",  "class"),
        ("CommunityEngine",   "class"),
        ("build_graph",       "function"),
        ("run_leiden",        "function"),
    ]:
        _hits = _find(_t, _sym)
        _check(f"Symbol '{_sym}' ({_t}) exists in graph", len(_hits) > 0,
               _hits[0][0] if _hits else "NOT FOUND")

    # Check that CodeGraphBuilder methods are class-prefixed
    _cbm = [nid for nid, d in g.nodes(data=True)
            if d.get("type") == "function"
            and (d.get("qualified_name") or "").startswith("CodeGraphBuilder.")]
    _check("CodeGraphBuilder methods have class-prefixed qualified names",
           len(_cbm) > 0, f"{len(_cbm)} found")

    # ── Phase 6: Save / load round-trip ──────────────────────────────────
    print(f"\n{_HEAD}[6] Save / load round-trip{_RST}")
    import tempfile, os as _os

    try:
        with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as _tf:
            _tmp = _tf.name
        builder.save_graph(_tmp)
        _size_kb = _os.path.getsize(_tmp) / 1024
        _check("save_graph() writes a non-empty file", _size_kb > 0, f"{_size_kb:.1f} KB")

        _b2 = CodeGraphBuilder()
        _b2.load_graph(_tmp)
        _g2 = _b2.graph
        _check("load_graph() restores identical node count",
               _g2.number_of_nodes() == n_nodes,
               f"got {_g2.number_of_nodes()} vs expected {n_nodes}")
        _check("load_graph() restores identical edge count",
               _g2.number_of_edges() == n_edges,
               f"got {_g2.number_of_edges()} vs expected {n_edges}")
        _os.unlink(_tmp)
    except Exception as _e:
        _check("save/load round-trip", False, str(_e))

    # ── Phase 7: Sample output ────────────────────────────────────────────
    print(f"\n{_HEAD}[7] Sample nodes{_RST}")
    print("  --- Top-level classes ---")
    for nid, d in sorted(g.nodes(data=True)):
        if d.get("type") == "class":
            bases = d.get("base_classes") or []
            print(f"    {nid:<60}  bases={bases}")

    print("\n  --- Functions with visibility ---")
    _sample_funcs = sorted(
        ((nid, d) for nid, d in g.nodes(data=True) if d.get("type") == "function"),
        key=lambda x: x[0]
    )[:15]
    for nid, d in _sample_funcs:
        print(f"    [{d.get('visibility','?'):9}] {d.get('qualified_name',''):<55}  "
              f"line {d.get('start_line','?')}-{d.get('end_line','?')}")

    # ── Summary ───────────────────────────────────────────────────────────
    print(f"\n{_HEAD}═══ Result ═══{_RST}")
    if _errors:
        print(f"  {_FAIL}  {len(_errors)} check(s) FAILED:")
        for _e_item in _errors:
            print(f"       • {_e_item}")
        sys.exit(1)
    else:
    ...
    # ── Phase 8: Export full graph JSON (NetworkX node-link format) ─────
    print(f"\n{_HEAD}[8] Export graph JSON{_RST}")
    import os as _os_save
    _out_json = _DOCKER_ROOT / "code_graph.json"
    try:
        builder.save_graph(str(_out_json))
        _kb = _os_save.path.getsize(_out_json) / 1024
        print(f"  {_PASS}  Saved: {_out_json}  ({_kb:.1f} KB)")
    except Exception as _e:
        print(f"  {_FAIL}  save_graph failed: {_e}")
        sys.exit(1)

...
*   **环境变量**:
    ...
    *   `VECTOR_STORE_PATH`: 向量库本地持久化路径（默认 `/data/vector_stores`，与 Fly.io 挂载一致）。
    *   `REPO_STORE_PATH`: 克隆仓库持久化路径（默认 `/data/repos`）。

### 5.1 持久化卷布局（本地 Docker 与 Fly.io 一致）

| 路径 | 用途 |
|------|------|
| `/data/vector_stores/<repo_name>/` | RAG 向量库（code/、text/ 子目录） |
| `/data/repos/<repo_hash>/` | 克隆的仓库源码（供 `/file/content` 读取） |

本地 `docker-compose` 挂载 `./data:/data`。首次运行请执行：
```bash
mkdir -p data/vector_stores data/repos
```
若曾使用旧版 `./vector_stores`，请迁移：
```bash
cp -r vector_stores/* data/vector_stores/
```
